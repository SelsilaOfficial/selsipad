...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (round.status !== 'APPROVED_TO_DEPLOY') {
      return NextResponse.json(
        { error: 'Round must be in APPROVED_TO_DEPLOY status' },
        { status: 400 }
      );
    }

    if (round.type !== 'PRESALE') {
      return NextResponse.json({ error: 'Only PRESALE rounds can be deployed via this endpoint' }, { status: 400 });
    }
    ...
    if (!projectOwnerAddress || !ethers.isAddress(projectOwnerAddress)) {
      return NextResponse.json(
        { error: 'No BSC Testnet wallet found for round owner. Owner must link a wallet for chain 97.' },
        { status: 400 }
      );
    }
    ...
    const startAt = round.start_at ? Math.floor(new Date(round.start_at).getTime() / 1000) : Math.floor(Date.now() / 1000);
    const endAt = round.end_at ? Math.floor(new Date(round.end_at).getTime() / 1000) : startAt + 7 * 24 * 3600;

    const paymentToken = (round.raise_asset === 'NATIVE' || !round.raise_asset)
      ? ethers.ZeroAddress
      : (round.raise_asset as string);
    if (paymentToken !== ethers.ZeroAddress && !ethers.isAddress(paymentToken)) {
      return NextResponse.json({ error: 'Invalid raise_asset (payment token) address' }, { status: 400 });
    }
    ...
    const dexId = lpLock.dexId ? ethers.keccak256(ethers.toUtf8Bytes(lpLock.dexId)) : ethers.id('PANCAKESWAP');
    ...
    if (!log) {
      return NextResponse.json({ error: 'PresaleCreated event not found in receipt' }, { status: 500 });
    }
    ...
    if (updateError) {
      ...
      return NextResponse.json({ error: 'Deployment succeeded but DB update failed' }, { status: 500 });
    }
