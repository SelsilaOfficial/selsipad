...
export const FairlaunchDeploySchema = z.object({
  // Token configuration
  ...
  dexPlatform: z.enum(['PancakeSwap', 'Uniswap', 'SushiSwap', 'BaseSwap'])
    .default('PancakeSwap')
    .describe('DEX platform for listing'),

  // Team vesting
  teamVestingTokens: z.string()
    .or(z.number())
    .transform((val: string | number) => String(val))
    .optional()
    .describe('Number of tokens allocated for team vesting'),

  teamVestingAddress: z.string()
  ...
  chainId: z.number()
    .int()
    .refine((id: number) => [97, 56, 11155111, 1, 84532, 8453].includes(id), 
